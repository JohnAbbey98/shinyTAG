...
.createMetapeaksPlot <- function(processed, metapeaks) {
  ...
  data <- .createData(processed, metapeaks)
  ...
  r <- ggplot() +
    theme_minimal() +
    ggtitle("Metapeaks") +
    geom_line(data = data$counts, aes(x = mz, y = count), color = 'black', alpha = 1) +
    geom_vline(data = data$targeted_metapeaks, aes(xintercept = expected_mz, text = name), color = 'firebrick2', size = 0.5) +
    geom_vline(data = data$untargeted_metapeaks, aes(xintercept = untargeted_mz), color = 'orange', linetype = 'dashed', size = 0.5) +
    geom_vline(data = data$targeted_metapeaks, aes(xintercept = metapeak_mz), color = 'springgreen3', linetype = 'dashed', size = 0.5) +
    geom_area(data = data$smooth_counts, aes(x = mz, y = count), fill = '#6062eb', alpha = 0.5) +
...
.createData <- function(processed, metapeaks){
  ...
  return(list(counts = counts,
              ...
              targeted_metapeaks = targeted_metapeaks,
              untargeted_metapeaks = untargeted_metapeaks))
